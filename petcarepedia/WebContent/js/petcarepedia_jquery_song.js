$(document).ready(function(){
	...
	//비밀번호 정규식 체크
	$("form[name='joinForm'] #pass").keyup(function(){
		if(!reg_pw.test($("form[name='joinForm'] #pass").val())){
	...
	//비밀번호 확인 유효성 체크
	$("form[name='joinForm'] #cpass").keyup(function(){
		if(!reg_pw.test($("form[name='joinForm'] #cpass").val())){
			$("#cpwcheck_msg").text("8~16자의 영문, 숫자, 특수문자를 포함하여 입력하세요.").css("color","red")
			.css("font-size","12px").css("display","block").css("clear","both")
			.css("padding-top","5px")
			.prepend("<img src='http://localhost:9000/petcarepedia/images/info_red.png' width='13px' style='padding-right:5px; vertical-align:middle'>");
		} else {
			if($("form[name='joinForm'] #cpass").val() == $("form[name='joinForm'] #pass").val()){
